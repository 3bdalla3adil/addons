...
class DentalSiteController(http.Controller):
    ...
    @http.route(['/get_appointment_list'], type='json', auth="none", methods=['GET', 'POST'], csrf=False))
    def get_appointment_list(self, **post):
        appt_ids = request.env['medical.appointment'].sudo().search([])
        return appt_ids.read([])
