...
class CalenderSaleWizard(models.TransientModel):
    _name = 'calender.sale.wizard'
    _description = "calender.sale.wizard"

    patient_id = fields.Many2one('medical.patient', 'Patient')
    doctor_id = fields.Many2one('medical.doctor', 'Doctor')
    clinic_id = fields.Many2one('medical.clinic', 'Clinic')
    service_ids = fields.One2many('app.service.line', 'wiz_saleorder_line', string='Services')
    appointment_id = fields.Many2one('medical.appointment')
    total_amount = fields.Float('Total Fees')

    @api.model
    def default_get(self, fields):
        res = super(CalenderSaleWizard, self).default_get(fields)
    ...
    def action_create_saleorder(self):
        ...
        saleorder_data = {
            ...
            'order_line': prods_lines
        }
        #invoice_id = saleorder_obj.create(invoice_data)
        saleorder_id = saleorder_obj.create(saleorder_data)
        #invoice_id.action_post()
        #saleorder_id.action_confirm()
        # self.appointment_id.invoice_id = invoice_id.id
        self.appointment_id.saleorder_id = saleorder_id.id
        self.appointment_id.is_invoice_state = True
        self.appointment_id.invoice_state = 'invoiced'

        message_id = self.env['medical.notes.message'].create({'name': _("Sale Order successfully created.")})
        # return {
        #     'name': _('Success'),
        ...
        return {
            ...
            'res_id': self.appointment_id.saleorder_id.id,
        }
...
class AppServiceLine(models.TransientModel):
    ...
    wiz_saleorder_line = fields.Many2one('calender.sale.wizard')
...
class CalenderAppointmentWizard(models.TransientModel):
    ...
    patient_id = fields.Many2one('medical.patient', 'Patient')
    # partner_id = fields.Many2one('res.partner', string="New Patient",
    ...
    #     for rec in self:
    #         if rec.partner_id:
    #             rec.patient_id.partner_id = rec.partner_id.id
    ...
    def action_create(self):
        ...
        if appointment_validation:
            ...
            #patient = self.env['medical.patient'].search([('id','=',self.patient_id.id)]).write({
            patient = self.patient_id.write({
