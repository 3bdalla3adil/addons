...
class ReportPatientByProcedure(models.AbstractModel):
    ...
    _name = 'report.beauty_clinic_management.report_patient_by_procedure'
    ...
    def get_patient_procedure(self, start_date, end_date):
        ...
        history_ids=self.env['account.move'].search([('invoice_date','>=',start_date),('invoice_date','<=',end_date),('clinic','!=',False),('state','in',['draft','posted']),('move_type','=','out_invoice')])
...
#         }
#         return self.env['report'].render('beauty_clinic_management.report_patient_by_procedure', docargs)
    ...
    def fetch_record(self, start_date, end_date):
        invoice_ids=self.env['account.move'].search([('invoice_date','>=',start_date),('invoice_date','<=',end_date),
                                                     ('clinic','!=',False),('state','in',['draft','posted']),('move_type','=','out_invoice')])

        res = []
        for each_record in invoice_ids:
            if not res:
                res.append({'clinic_id': each_record.clinic.id, 'clinic_name': each_record.clinic.name,
                ...
                for each_res in res:
                    if each_record.clinic.id == each_res['clinic_id']:
                        ...
                        break
                if flag == 0:
                    res.append({'clinic_id': each_record.clinic.id, 'clinic_name': each_record.clinic.name.name,
                                'customer_count': 1, 'total_amount': each_record.amount_total})
